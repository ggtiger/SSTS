...
SSTS Tauri 桌面应用支持 macOS（ARM64/x64）、Windows x64、Linux x64 三平台，通过 GitHub Actions 自动打包发布。
...
| `APPLE_CERTIFICATE_PASSWORD` | .p12 密码 | `ssts2026` |
...
base64 -i dev-id.p12 | tr -d '\n' | gh secret set APPLE_CERTIFICATE -R ggtiger/SSTS
echo -n "YOUR_PASSWORD" | gh secret set APPLE_CERTIFICATE_PASSWORD -R ggtiger/SSTS
```
...
| 平台 | 文件 |
|------|------|
| macOS ARM64 | `SSTS_0.1.0_aarch64.dmg` |
| macOS x64 | `SSTS_0.1.0_x64.dmg` |
| Windows x64 | `SSTS_0.1.0_x64-setup.exe`、`SSTS_0.1.0_x64_en-US.msi` |
| Linux x64 | `SSTS_0.1.0_amd64.deb`、`SSTS-0.1.0-1.x86_64.rpm` |
