 ...
 * deploy-build.js
 * 
 * 生成可独立部署的 SSTS 服务端包（tar.gz）。
 * 产物可直接部署到任何有 Node.js 的服务器上。
 * 
 * 用法: node scripts/deploy-build.js
 * 产物: dist/ssts-server.tar.gz
 * 
 * 部署到服务器后:
 *   tar xzf ssts-server.tar.gz
 *   cd ssts-server
 ...
 *   SSTS_DATA_DIR - 数据目录 (默认 ./data 相对于 server.js)
 */
...
const DEPLOY_DIR = path.join(DIST_DIR, 'ssts-server')
const TARBALL = path.join(DIST_DIR, 'ssts-server.tar.gz')

// 1. 构建 standalone 并定位
const standaloneRoot = buildAndLocateStandalone()

// 2. 组装到 dist/ssts-server/
...
#   PORT          - 监听端口 (默认 3000)
#   HOSTNAME      - 绑定地址 (默认 0.0.0.0)
#   SSTS_DATA_DIR - 数据持久化目录 (默认 当前目录下的 data/)

cd "$(dirname "$0")"

export PORT=\${PORT:-3000}
export HOSTNAME=\${HOSTNAME:-0.0.0.0}
export SSTS_DATA_DIR=\${SSTS_DATA_DIR:-$(pwd)/data}

echo "[SSTS] Starting server on \${HOSTNAME}:\${PORT}"
echo "[SSTS] Data directory: \${SSTS_DATA_DIR}"
...
execSync(`tar czf "${TARBALL}" -C "${DIST_DIR}" ssts-server`, { stdio: 'inherit' })
...
console.log('')
console.log('=== 部署说明 ===')
console.log('1. 将 ssts-server.tar.gz 上传到服务器')
console.log('2. 解压: tar xzf ssts-server.tar.gz')
console.log('3. 启动: cd ssts-server && bash start.sh')
console.log('   或: PORT=8080 HOSTNAME=0.0.0.0 node server.js')
console.log('')
console.log('环境变量:')
console.log('  PORT           监听端口 (默认 3000)')
console.log('  HOSTNAME       绑定地址 (默认 0.0.0.0)')
console.log('  SSTS_DATA_DIR  数据目录 (默认 ./data)')
